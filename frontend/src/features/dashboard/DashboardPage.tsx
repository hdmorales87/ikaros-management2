...
export default function DashboardPage() {
  ...
  const results = useQueries({ queries: cards.map((card) => ({ queryKey: ['dashboard', card.table, card.filter], queryFn: () => getGridRows(card.table, '', card.filter, card.fields) })) })
  return <section><div className="topbar"><div><h1>Resumen operativo</h1><p className="muted">{session?.companyData?.razon_social || 'Tu organización'}</p></div></div><div className="dashboard-grid">{cards.map((card, index) => <article className={`metric metric-${card.tone}`} key={card.label}><span className="metric-label">{card.label}</span><strong>{results[index].isLoading ? '...' : results[index].data?.length ?? 0}</strong><span className="muted">registros visibles</span></article>)}</div><div className="panel dashboard-note"><h2>Centro de trabajo</h2><p className="muted">Usa la navegación para consultar y gestionar la información de cada módulo.</p></div></section>
}
