...
const cards = [
  { key: 'incidents', label: 'Incidencias', tone: 'blue' },
  { key: 'services', label: 'Servicios', tone: 'teal' },
  { key: 'assets', label: 'Activos', tone: 'green' },
  { key: 'projects', label: 'Proyectos', tone: 'violet' },
  { key: 'trainings', label: 'Capacitaciones', tone: 'amber' },
] as const

export default function DashboardPage() {
  const { session } = useAuth()
  const summary = useQuery({ queryKey: ['v1-dashboard-summary'], queryFn: dashboardApi.summary })

  const total = Object.values(summary.data ?? {}).reduce((sum, value) => sum + value, 0)
  ...
  return <section>
    ...
    <div className="dashboard-grid">
      {cards.map((card, index) => <article className={`metric metric-${card.tone}`} key={card.label}>
        ...
        <strong>{summary.isLoading ? '...' : summary.data?.[card.key] ?? 0}</strong>
